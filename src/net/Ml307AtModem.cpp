...
Ml307AtModem::Ml307AtModem(HardwareSerial &serial, int rxPin, int txPin)
    : _serial(serial), _rxPin(rxPin), _txPin(txPin), _gnssEnabled(false), _lbsEnabled(false), _debug(false), _lastLBSLocation("")
{
}
...
{
    if (millis() - _lastGNSSUpdate > 1000)
    { // 每秒更新一次
        updateGNSSData();
    }
    return _gnssData;
...
{
    return _lastLBSLocation; // 直接返回最近一次的位置信息
}
...
bool Ml307AtModem::updateLBSData()
{
    ...
    // 使用线程安全的AT命令发送
    if (!sendATThreadSafe("AT+MLBSLOC", "OK", 5000))
    {
        debugPrint("LBS命令发送失败");
        _isLBSLoading = false;
        return false;
    }
...
void Ml307AtModem::processURC(const String &urc)
{
    debugPrint("处理URC: " + urc);
    // 处理LBS位置信息
    if (urc.startsWith("+MLBSLOC: "))
    {
        _lastLBSLocation = urc;
        debugPrint("LBS位置信息: " + _lastLBSLocation);
        // 解析位置信息
        int firstComma = urc.indexOf(',');
        if (firstComma > 0)
        {
            String status = urc.substring(10, firstComma); // 跳过"+MLBSLOC: "
            if (status == "100")
            { // 成功状态
                int secondComma = urc.indexOf(',', firstComma + 1);
                if (secondComma > 0)
                {
                    String longitude = urc.substring(firstComma + 1, secondComma);
                    String latitude = urc.substring(secondComma + 1);
                    debugPrint("LBS定位成功 - 经度: " + longitude + ", 纬度: " + latitude);
                }
            }
        }
    }
