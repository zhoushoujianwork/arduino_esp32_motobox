...
TFT::TFT(uint8_t rotation)
{
    this->rotation = rotation;
    this->initialized = false;
    this->buf = new lv_color_t[TFT_VER_RES * TFT_HOR_RES / 10];
...
void TFT::begin()
{
    ...
    disp_drv.flush_cb = disp_flush_cb; // 使用静态回调函数
    ...
    indev_drv.read_cb = touchpad_read_cb; // 使用静态回调函数
...
/* Display flushing */
void TFT::disp_flush_cb(lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_p)
{
    instance->my_disp_flush(disp, area, color_p);
}

/*Read the touchpad*/
void TFT::touchpad_read_cb(lv_indev_drv_t *indev_driver, lv_indev_data_t *data)
{
    instance->my_touchpad_read(indev_driver, data);
}

void TFT::my_disp_flush(lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_p)
{
...
/*Read the touchpad*/
void TFT::my_touchpad_read(lv_indev_drv_t *indev_driver, lv_indev_data_t *data)
{
